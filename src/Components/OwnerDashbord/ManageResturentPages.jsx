...
const initialFormData = {
  ...
  deliveryTime: " ",
  timing: { open: "", close: "" },
  location: { address: "", city: "", pincode: "", area: "" },
  offers: [],
  images: [],
  tags: [],
  popularDishes: "",
...
const RestaurantForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      timing: { open: formData.openTime, close: formData.closeTime },
    ...
    try {
      ...
      if (editingRestaurant) {
        ...
        await axios.post(`${baseURL}/createrestaurant`, data, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
  ...
  return (
    <form
      ...
      className="bg-white shadow p-4 md:p-6 rounded-lg space-y-4 mb-10"
    >
...
const RestaurantList = ({
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 md:p-6 rounded-lg shadow">
      <h1 className="text-2xl font-bold mb-6 text-center md:text-left">
        Top Restaurants
      </h1>

      <div className="flex flex-wrap justify-center md:justify-start gap-5 overflow-x-auto scroll-smooth [&::-webkit-scrollbar]:hidden">
        {restaurants.length > 0 ? (
          restaurants.map((restro) => (
            <div
              key={restro._id}
              onClick={() => navigate(`/restaurant/${restro._id}`)}
              className="w-[260px] sm:w-[280px] hover:scale-105 cursor-pointer shadow-xl rounded-xl p-2 bg-white flex-shrink-0"
            >
              <img
                src={restro.images?.[0]}
                alt={restro.name}
              ...
              <button
                ...
                className="bg-yellow-500 text-white w-full mt-2 py-1 rounded hover:bg-yellow-600"
              >
                Edit
              </button>
            </div>
          ...
          <p className="text-gray-500 text-center">Loading restaurants...</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-4 mt-6">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4">
        <Link to="/restourentdasboard" className="mb-2 sm:mb-0">
